...
def eval_model(nodes, num_nodes, hnet, net, criteria, device, split, last=False):
    curr_results = evaluate(nodes, num_nodes, hnet, net, criteria, device, split=split, last=last)
...
@torch.no_grad()
def evaluate(nodes: BaseNodes, num_nodes, hnet, net, criteria, device, split='test', last=False):
    hnet.eval()
        ...
        for batch_count, batch in enumerate(curr_data):
            ...
            if last:
                ...
                for x in range(len(pred)):
                    plt.imshow(img[x].numpy().T)
                    plt.savefig(f'pred_res/prediction{pred.argmax(1).numpy()[x]}_labels{label.numpy()[x]}.png')
                    plt.close()
...
def train(data_name: str, data_path: str, classes_per_node: int, num_nodes: int,
    ...
    if data_name == "cifar10":
        ...
        hnet = CNNHyper(num_nodes, embed_dim, hidden_dim=hyper_hid,
                        n_hidden=n_hidden, n_kernels=n_kernels, out_dim=38)
        net = CNNTarget(n_kernels=n_kernels, out_dim=38)
    else:
    ...
    if step != last_eval:
        _, val_avg_loss, val_avg_acc, _ = eval_model(nodes, num_nodes, hnet, net, criteria, device, split="val")
        step_results, avg_loss, avg_acc, all_acc = eval_model(nodes, num_nodes, hnet, net, criteria, device, split="test", last=True)
        logging.info(f"\nStep: {step + 1}, AVG Loss: {avg_loss:.4f},  AVG Acc: {avg_acc:.4f}")
    ...
    print('-------------------done----------------')
    #x = np.arrange(len(results['test_avg_loss']))
    #x = results['step_num'].append(step)
    x = results['step_num']
    print(len(x))
    print(len(results['tes_avg_loss']))
    plres = defaultdict(list)
    prev_tal = 0
    prev_taa = 0
    prev_val = 0
    prev_vaa = 0
    for i in range(len(results['step_num'])):
        prev_tal = results['test_avg_loss'][i]
        prev_taa = results['test_avg_acc'][i]
        prev_val = results['val_avg_loss'][i]
        prev_vaa = results['val_avg_acc'][i]
        plres['tal'].append(prev_tal)
        plres['taa'].append(prev_taa)
        plres['val'].append(prev_val)
        plres['vaa'].append(prev_vaa)
        
    plt.plot(x, plres['tal'])
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Test Average Loss')
    plt.savefig('test_ave_loss.png')
    plt.clf()

    plt.plot(x, plres['taa'])
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title('Test Average Accuracy')
    plt.savefig('test_ave_acc.png')
    plt.clf()

    plt.plot(x, plres['val'])
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Validation Average Loss')
    plt.savefig('val_ave_loss.png')
    plt.clf()

    plt.plot(x, plres['vaa'])
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title('Test Average Accuracy')
    plt.savefig('val_ave_acc.png')
    plt.clf()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num-nodes", type=int, default=25, help="number of simulated nodes")
    #parser.add_argument("--num-nodes", type=int, default=50, help="number of simulated nodes")
    #parser.add_argument("--num-nodes", type=int, default=5, help="number of simulated nodes")
    ...
    parser.add_argument("--eval-every", type=int, default=3, help="eval every X selected epochs")
    ...
    elif args.data_name == 'pd':
        args.classes_per_node = 5
    else:
